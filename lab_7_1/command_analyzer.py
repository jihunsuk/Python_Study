...
for key, value in command_counter.items():
    temp = [key,value]
    dictlist.append(temp) # id별로 몇번 나왔는지 저장

sorted_dict= sorted(dictlist, key=getKey, reverse=True) # list를 입력 줄 수로 정렬
print (sorted_dict[:100]) # List의 상위 100객값만 출력


# sorted는 문자열 또는 리스트, 튜플을 파라미터로 받아 리스트로 반환해준다.
# reverse = True면 내림차순 정렬 default값은 False 오름차순이다.
# sorted의 key는 정렬 기준값을 생성할 함수이다. 즉 getKey이므로 dictlist의 key로 정렬한다.
